...
class AttrData {
    constructor(query, render) { this.type = query.replace("\\", "").replace("\$", "");
        this.query = query;
        this.render = render }
}
class SpecialAttr {
    constructor(type, element, render) { this.attr = type;
...
function renderIf(expression, element) {
    ...
    let result = eval(exp);
    if (result) { if (element.classList.contains('hide')) { element.classList.remove('hide') } } else { if (!element.classList.contains('hide')) { element.classList.add('hide') } }
}
...
function renderFor(exp, element) {
    ...
    let newElement = "";
    for (let i = 0; i < array.length; i++) { let tempele = element.innerHTML.replace(new RegExp("\[$]" + subArr, 'g'), "$" + arrName + `[${i}]`).replace(iterregx, i); let oldval = $scope[subArr];
        $scope[subArr] = array[i]; var node = document.createElement("LI");
...
function renderDisabled(exp, element) { exp = exp.replace(/\$/g, "$scope."); let result = eval(exp);
...
function renderStyle(exp, element) {
    let hashCode = function(str) {
        ...
        for (var i = 0; i < str.length; i++) { var char = str.charCodeAt(i);
            hash = ((hash << 5) - hash) + char;
            hash = hash & hash }
        return hash
    }
    let expGroup = exp.split(',');
    expGroup.forEach(ele => { let indx = ele.lastIndexOf(":"); let e = [];
        e.push(ele.substring(0, indx).replace(/'/g, "").trim(), ele.substring(indx + 1).trim()); let h = "mvc" + hashCode(e[0]);
        createClass(h, e[0]);
...
function specialTags(doc) {
    let specialTags = [];
    for (let i = 0; i < specails.length; i++) { let elements = doc.querySelectorAll(`[${specails[i].query}]`);
...
function replaceElement(attrString) { let value = attrString.replace('$', "$scope.");
    value = eval(value); return value || '' }
export function $apply(doc) {
    let str;
    if (!doc) { str = $bindedVars;
        doc = viewElement } else { str = doc.innerHTML }
...
export function render(view, model) { let specials = specialTags(view);
    $scope = model;
    specials.forEach(element => element.render(element.exp, element.element));
    $bindedVars = view.innerHTML;
    $apply(view); return view }

function renderTemplate(view) { let specials = specialTags(view);
    specials.forEach(element => element.render(element.exp, element.element)); return view }