...
function renderIf(expression,element){try{let exp=expression.replace(/\$/g,"$scope.");createClass("hide","display: none");let result=eval(exp);result?element.classList.contains("hide")&&element.classList.remove("hide"):element.classList.contains("hide")||element.classList.add("hide")}catch(e){console.log(e)}}
